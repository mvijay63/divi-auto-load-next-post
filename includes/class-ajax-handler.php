...
class Divi_Auto_Load_Ajax_Handler {
                ...
                if (preg_match('/href=["\']([^"\']+)["\']/', $link_tag, $href_match)) {
                    ...
                    if (strpos($css_url, 'et-cache') !== false || strpos($css_url, 'divi-dynamic') !== false) {
                        ...
                        if (!is_wp_error($css_response)) {
                            ...
                            $scoped_css .= $this->scope_css_with_post_id($css_content, $post_id) . "\n";
                        }
                    }
                }
            }
     ...
     * Only scopes et_pb_ builder rules — does NOT scope et- theme rules,
     * which live in the global stylesheet and must remain unmodified.
     */
